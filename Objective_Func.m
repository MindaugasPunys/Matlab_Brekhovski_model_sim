...
function error = Objective_Func(args, ref_signal, meas_signal, params, ...
    f_axis, time, H_electronics, start_point, end_point, model)
model_signal = Wave_synthesize(args, ref_signal, params, ...
    f_axis, time, H_electronics, model);
...
function Wave = Wave_synthesize(args, input_signal_param, params, ...
    freq, time, H_electronics, model)
RespSpectrum = Response_Spectrum(args, input_signal_param, params, ...
    freq, time, H_electronics, model);
Wave = real(ifft(RespSpectrum));

end

function RespSpectrum = Response_Spectrum(args, input_signal_param, params, ...
    freq, time, H_electronics, model)
global AirArguments
